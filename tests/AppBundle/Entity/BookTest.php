...
class BookTest extends KernelTestCase
{
    ...
    public function testBookHasAPublisher()
    {
        ...
        $result = $this->em->getRepository(Book::class)
                    ->find($book->getId());

        $this->assertInstanceOf('AppBundle\Entity\Publisher', $result->getPublisher());
        $this->assertEquals($publisher->getId(), $result->getPublisher()->getId());
        $this->assertEquals($publisher->getname(), $result->getPublisher()->getName());
    }

    public function testBookBelongsToBranch()
    {
        $publisher = $this->createPublisher();
        $branch = $this->createBranch();
        $book = $this->createBook($publisher, $branch);
        ...
        $this->assertEquals($branch->getId(), $result->getBranch()->getId());
        $this->assertEquals($branch->getName(), $result->getBranch()->getName());
    }

    public function testBookKnowsIssueData()
    {
        $publisher = $this->createPublisher();
        $branch = $this->createBranch();
        $book = $this->createBook($publisher, $branch);
